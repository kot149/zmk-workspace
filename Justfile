...
draw:
    #!/usr/bin/env bash
    set -euo pipefail
    keymap -c "{{ draw }}/config.yaml" parse -z "{{ config }}/{{ zmk_config }}/config/roBa_L.keymap" --virtual-layers Combos |
        yq -Y '.combos.[].l = ["Combos"]' |
        tee "{{ draw }}/base.yaml" |
        keymap -c "{{ draw }}/config.yaml" draw - -d "{{ config }}/{{ zmk_config }}/boards/shields/roBa/roBa.dtsi" >"{{ draw }}/base.svg"
